...
func SendUnicastResponse(payload []byte, targetAddr net.Addr) error {
	...
	if UDPConn != nil {
		if logger.IsDebugActive("wsdd") {
			logger.DebugF("wsdd", "Reusing central socket to transmit unicast response out of port 3702 to: %s", udpAddr.String())
		}
		n, err := UDPConn.WriteToUDP(payload, udpAddr)
		if err != nil {
			logger.ErrorF("wsdd", "Network write operation failure via central socket: %s", err, targetString)
			return err
		}
		logger.InfoF("wsdd", "Successfully dispatched %d bytes from source port 3702 to client: %s", n, targetString)
		return nil
	}
...
func SendMulticastBroadcast(payload []byte) error {
	logger.DebugF("wsdd", "Preparing outbound multicast protocol transmission loop for address target: %s:%s", DiscoveryMulticastIP, DiscoveryMulticastPort)
	...
	if UDPConn != nil {
		logger.DebugF("wsdd", "Reusing central socket to write multicast broadcast to group: %s:%s", DiscoveryMulticastIP, DiscoveryMulticastPort)
		...
		logger.InfoF("wsdd", "Successfully broadcasted %d bytes over multicast channel to: %s:%s", n, DiscoveryMulticastIP, DiscoveryMulticastPort)
		return nil
	}
